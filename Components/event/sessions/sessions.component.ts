...
export class SessionsComponent implements OnInit {
  @Input() readableEventId: string;
    sessions: Session[];
    sessiontimestrings: SessionTimeStrings[];
  hours: number[];
  schedule: Schedule;
  sessionsByDay: any;
  dateKeys: string[];
  ...
  ngOnInit() {
    ...
    private loadSessions(): void {
        this.eventService.getSessionTimeStrings(this.readableEventId)
            .subscribe(sessiontimestrings => {
                this.sessiontimestrings = sessiontimestrings.items;
                this.marrySessionsAndSessionTimeStrings();
            },
        ...
        this.eventService.getSessions(this.readableEventId)
            .subscribe(sessions => {
                ...
                this.sessions.forEach(s => {
                    ...
                    s.startTimeString = "Loading...";
                    s.endTimeString = "Loading...";
                });
                this.sessionsByDay = {};
                this.dateKeys = [];
  ...
  private generateTimelinesForDay(sessionsInDay: Session[]): Schedule {
    ...
    private marrySessionsAndSessionTimeStrings(): void {
        if (this.sessions != null && this.sessions != undefined) {
            for (const session of this.sessions) {
                if (this.sessiontimestrings != null && this.sessiontimestrings != undefined) {
                    for (const sessiontimestrings of this.sessiontimestrings) {
                        if (session.id == sessiontimestrings.id) {
                            session.startTimeString = sessiontimestrings.startTimeString;
                            session.endTimeString = sessiontimestrings.endTimeString;
                        }
                    }
                }
            }
